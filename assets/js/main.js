...
INFO = {
    ...
    { imagePath: './assets/speaker_data/nick_pentreath/image.jpg',
        ...
        category: 'Industry/Ethics/Policy',
        affiliation: 'IBM',
        abstract: "The common perception of applying deep learning is that you take an open source or research algorithm, train it on raw data, and deploy the resulting model. The reality is far more complex. For the training phase, users face an array of challenges including handling varied deep learning frameworks, hardware requirements and configurations, not to mention code quality, consistency, and packaging. For the deployment phase, they face another set of challenges, ranging from custom requirements for data pre- and post-processing, to inconsistencies across frameworks, to lack of standardization in serving APIs. The goal of the IBM Developer Model Asset Exchange (MAX) is to remove these barriers to entry for developers to obtain, train, and deploy open source deep learning models for their business applications. In building the exchange, we encountered all these challenges and more. For the training phase, MAX uses the Fabric for Deep Learning (FfDL), an open source project providing framework-independent training of deep learning models on Kubernetes. For the deployment phase, MAX provides standardized container-based, fully self-contained model artefacts, using Python web and DL frameworks and encompassing the end-to-end deep learning predictive pipeline. This talk introduces MAX, walks through the process of building MAX and shares the challenges and problems encountered, the solutions developed, and the lessons learned. The talk will also include live demos showing how to use MAX to deploy deep learning models in seconds and incorporate them into real-world applications.",
        bio: ""
    },
};
